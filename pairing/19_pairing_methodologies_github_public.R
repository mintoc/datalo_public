...
## Connect to database
## Assigned path connection to database as "con"

## ---Foyle Fisher T90 90mm and diamond 80mm 120mm SMP-----------------------------------
## Import length data (date, tow_id, gear, compartment, species, length_id, count_n)
dat_lengths <- sqlQuery(con, "SELECT *
                               FROM 201905_T90_90mm_V_T0_80mm_120mmSMP_lengths;")

## Import weights data (date, tow_id, gear, compartment, species, subsample, weight)
dat_weights <- sqlQuery(con, "SELECT *
                               FROM 201905_T90_90mm_V_T0_80mm_120mmSMP_weights;")

## Import haul data (tow_id, tow_valid)
...
run_boot_fn <- function(sppi, n_boot){
  ...
  for(j in 1:n_boot){
    ...
    ## NB Dont factor tows before this point
    tows <- as.integer(unique(dat$tow_id))
    ## Number of paired hauls
    ## 10 valid tow pairs for this trial
    J <- length(tows)  
    ...
    for(k in 1:J){
      ...
      ## Make data frame for experimental
  ...
  ## Populate dataframe with 95% CI's and SE data
...
for(i in 1:n_spp){
  ...
  ## Run bootstrap, adjust n_boot for number of runs
...
##----------------- Paired haul analysis as experimentally planned
...
## If no subsratio given, assume subsample ratio of 1
...
## Reassign tow_id's to conform with existing script
